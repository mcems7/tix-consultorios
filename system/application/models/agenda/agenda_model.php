...
////////////////////////////////////////////////////////////////////////////////
	...
	$this->db->FROM('agenda_parametros');
 ...
 function datosAgenda($fecha)
 {
     ...
     $this->db->SELECT('adt.id_agenda_consultorio, aadc.id_consultorio,
      ...
      $this->db->FROM('agenda_dia_detalle adt');
      ...
      $this->db->JOIN('agenda_agenda_dia_consultorio aadc ','aadc.id_agenda_consultorio= adt.id_agenda_consultorio');
      $this->db->JOIN('agenda_agenda_dia aad','aad.id_agenda_dia=aadc.id_agenda_dia');
                                    ...
                                    (SELECT sum(duracion) 
                                     ...
                                     WHERE a.total_horas_mes<>0 and a.estado=1
                                           AND cm.id_especialidad in (SELECT tf.id_especialidad
                                                                      FROM agenda_consultorios_especialidades tf
 ...
 function agenda_dia($dia)
 {
     ...
     $this->db->FROM('agenda_agenda_dia');
 ...
 function agenda_consultorio_dia($dia,$id_consultorio)
 {
     ...
     if(count($agenda_dia)!=0)
     {
         ...
         $this->db->FROM('agenda_agenda_dia_consultorio');
 ...
 function agregar_agenda_consultorio_dia($id_agenda_dia, $id_consultorio)
 {
       ...
       $this ->db->insert('agenda_agenda_dia_consultorio',$insert);
  ...
  function agregar_agenda_dia($dia)
 {
    $d=array(
     ...
     $this ->db->insert('agenda_agenda_dia',$d);
 }
 function agregar_detalle_agenda($id_medico, $id_consultorio, $intervalo, $dia,$hora,$duracion)
 {
     ...
     print_r($insert);
     $this->db->insert('agenda_dia_detalle',$insert);
 }
 ...
 function agenda_consultorio_dia_no($dia, $id_consultorio)
 {
    return $this->db->query(
            "SELECT id_agenda_consultorio from agenda_agenda_dia_consultorio a
            WHERE a.id_agenda_dia in (SELECT b.id_agenda_dia 
                                      FROM  agenda_agenda_dia b 
 ...
 {
     return $this->db->query(
                "SELECT a.id_agenda_dia_detalle from agenda_dia_detalle a
                 WHERE a.id_agenda_consultorio in (
                                SELECT b.id_agenda_consultorio
                                FROM agenda_agenda_dia_consultorio b
                                WHERE b.id_agenda_dia in(SELECT c.id_agenda_dia 
                                     			 FROM  agenda_agenda_dia c 
...
from agenda_dia_detalle a 
         inner join `agenda_agenda_dia_consultorio` b
		 on a.`id_agenda_consultorio`=b.`id_agenda_consultorio`
         inner join `agenda_agenda_dia` c 
         on b.`id_agenda_dia`= c.`id_agenda_dia`
         inner join `agenda_consultorios` d
 ...
 function lista_especialidades_contratadas()
 {
     ...
     $this->db->FROM('agenda_consultorios_especialidades ace');
 ...
 function agenda_medicos($fecha_inicial, $fecha_final, $id_especialista)
 {
     ...
     $this->db->distinct();
     $this->db->FROM("agenda_agenda_dia agd");
     $this->db->JOIN("agenda_agenda_dia_consultorio agdc","agd.id_agenda_dia = agdc.id_agenda_dia");
     $this->db->jOIN("agenda_consultorios ac","ac.id_consultorio = agdc.id_consultorio");
     $this->db->JOIN("agenda_dia_detalle agdd","agdd.id_agenda_consultorio=agdc.id_agenda_consultorio");
 ...
 function suspender_citas_agenda_intervalos($fecha, $id_consultorio, $intervalo_inicial, $intervalo_final)
 {
     $consulta="SELECT caa.`id_remision`
        ...
        inner join `agenda_agenda_dia_consultorio` aadc on aadc.`id_agenda_consultorio`=agdd.`id_agenda_consultorio`
        inner join `agenda_agenda_dia` aad on  aad.`id_agenda_dia`=aadc.id_agenda_dia
     ...
     if(count($resultado)!=0)
     {
         ...
         $this->db->DELETE('citas_asignadas_agenda');
         
     }
    $select="select `agenda_dia_detalle`.`id_agenda_dia_detalle` id
            from  `agenda_dia_detalle` 
            inner join `agenda_agenda_dia_consultorio` aadc ON agenda_dia_detalle.`id_agenda_consultorio`=aadc.`id_agenda_consultorio`
            inner join `agenda_agenda_dia` aagd ON aadc.`id_agenda_dia`=aagd.`id_agenda_dia`
            WHERE aagd.`fecha`='$fecha' and 
                        (`agenda_dia_detalle`.`orden_intervalo` between $intervalo_inicial and $intervalo_final) and
   ...
   if(count($resultado)!=0)
   {
       ...
       $this->db->DELETE('agenda_dia_detalle');
   }
 }
 //////////////////////////////////////////////////////////////////////////////
 //
 ...
 function medico_ocupa_consultorio_hora($id_medico, $intervalo_inicial,$intervalo_final,$fecha)
 {
     $query="select count(*) total
            from `agenda_agenda_dia` aad
            inner join `agenda_agenda_dia_consultorio` aadc on aad.`id_agenda_dia`=aadc.`id_agenda_dia`
            inner join `agenda_dia_detalle` agdd on aadc.`id_agenda_consultorio`=agdd.`id_agenda_consultorio`
