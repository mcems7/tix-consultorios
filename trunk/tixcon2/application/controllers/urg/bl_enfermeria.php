...
class Bl_enfermeria extends CI_Controller
{
///////////////////////////////////////////////////////////////////
...
*/
	...
	function main($id_atencion)
	{
		...
		$d['notas'] = $this -> enfermeria_model -> obtenerBlNotasEli($id_atencion);
		if ($d['notas'] == 0)
		{
		  $d['notas'] = $this -> enfermeria_model -> obtenerBlNotasAdm($id_atencion);
		}
...
$this->graph->set_output_type('js');
		...
		$this -> load -> view('urg/urg_blListado', $d);
...
*/
	function consultaBalance($id_atencion,$fecha_nota)
	{
		...
		echo $this->load->view('urg/urg_BLnotaConsulta',$d);
	}
///////////////////////////////////////////////////////////////////
/* 
...
*/
	function crearNotaAdm($id_atencion)
	{
		...
		$this -> load -> view('urg/urg_blnotaCrear',$d);
...
*/
		...
		function crearNotaEli($id_atencion)
	{
		...
		$this -> load -> view('urg/urg_blnotaCrearEliminados',$d);
...
*/
	function crearNotaBlAdm_()
	{
		...
		//----------------------------------------------------------
		$d['id_nota'] = $this -> enfermeria_model -> crearBl_admi($d);
		//----------------------------------------------------
		$dt['mensaje']  = "Los datos de balance de liquidos  se han almacenado correctamente!!";
		$dt['urlRegresar'] 	= site_url("urg/bl_enfermeria/main/".$d['id_atencion']);
...
function crearNotaBlElimi_()
	{
		...
		//----------------------------------------------------------
		$d['id_nota'] = $this -> enfermeria_model -> crearBl_elimi($d);
		//----------------------------------------------------
		$dt['mensaje']  = "Los datos de balance de liquidos  se han almacenado correctamente!!";
		$dt['urlRegresar'] 	= site_url("urg/bl_enfermeria/main/".$d['id_atencion']);
		$this -> load -> view('core/presentacionMensaje', $dt);
...
$this->graph->set_output_type('js');
		...
		$this -> load -> view('urg/urg_blListadoEfecto', $d);
		$this->load->view('core/core_fin');
		//----------------------------------------------------------
