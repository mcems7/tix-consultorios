...
*/
class Hce_consulta_model extends CI_Model 
{
//////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function obtenerAtencionesPacientes($d)
{
    ...
    $this->db->join('cita_remisiones cr',' cr.id_tercero=ct.id_tercero');
    ...
    $this->db->join('agenda_dia_detalle agdd','agdd.id_agenda_dia_detalle= caa.id_agenda_dia_detalle');
    $this->db->join('agenda_agenda_dia_consultorio aadc','aadc.id_agenda_consultorio = agdd.id_agenda_consultorio');
...
function incapacidad_paciente($id_cita)
{
    ...
    $this->db->FROM('atencion_incapacidad ai');
...
function plan_y_manejo_paciente($id_cita)
{
    ...
    $this->db->FROM('citas_asignadas_agenda');
...
function diagnosticos_paciente($id_cita)
{
    ...
    $this->db->FROM('atencion_diagnostico ad');
...
{  
    $this->db->SELECT('cm.principio_activo, cm.descripcion, 
                       ...
                       observacionesMed');
    $this->db->FROM('atencion_orde_medicamentos aom');
...
function ayudas_diagnosticas($id_cita)
{
    ...
    $this->db->FROM('atencion_orde_cups` aoc');
...
function cargar_items_hce($id_especialidad)
{
    ...
    $this->db->FROM('atencion_item_hc aiha');
    $this->db->WHERE("aiha.idpadre=(select id from atencion_item_hc aih 
...
function remision($id_cita)
{
    $this->db->SELECT('motivo_remision,observacion,otra_especialidad,descripcion especialidad,
                       tipo_cita');
    $this->db->FROM('atencion_remision ar');
...
function control($id_cita)
{
    ...
    $this->db->FROM('atencion_remision ar');
    $this->db->JOIN('cita_remisiones cr','cr.id=ar.id_atencion');
    $this->db->WHERE('id_atencion',$id_cita);
