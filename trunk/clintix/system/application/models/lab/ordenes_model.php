...
$this->db->update('lab_resultados_detalle', $update2); 
	...
	
	{
			...
			$this->db->limit(1);
			$this->db->order_by('lab_ordenes.id_ordenes', 'desc'); 
				$result = $this->db->get();
				$num = $result->num_rows();
	...
	/////////////////////////////////////////////////
	//traemos los resultados del analizador para la muestra indicada.
	
	function OrdenInterpretarAnalizador($id_ordenes)
	{
		$this->db->select('	
		 lab_resultados_analizadores.codigo,lab_resultados_analizadores.nombre,lab_resultados_analizadores.valor');
			$this->db->from('lab_resultados_analizadores');
			$this->db->where('lab_resultados_analizadores.id_orden',$id_ordenes);
				$result = $this->db->get();
				$num = $result->num_rows();
				if($num == 0)
				{
