...
//////////////////////////////////////////////////////////////////////////////////
    function especialistas_programados_dia($fecha, $id_especialidad)
    {
       $query="select distinct cm.`id_medico`, ct.`primer_apellido`, 
               ct.`segundo_apellido`, ct.`primer_nombre`, ct.`segundo_nombre`, ad.bloqueado
               from `cex_agenda_dia_detalle`  ad
               ...
               WHERE ad.`id_agenda_consultorio` in (                                    
                                    ...
                                    from `cex_agenda_agenda_dia_consultorio` aadc
                                    where aadc.`id_agenda_dia` in (
                                          select aad.`id_agenda_dia` 
                                          from`cex_agenda_agenda_dia` aad where fecha='$fecha'))
...
//////////////////////////////////////////////////////////////////////////////////
    function especialidades_programadas_dia($fecha)
    {
        ...
        $this->db->DISTINCT();
        $this->db->FROM('cex_agenda_dia_detalle agdt ');
        $this->db->JOIN('cex_agenda_agenda_dia_consultorio agdc','agdt.id_agenda_consultorio= agdc.id_agenda_consultorio');
        $this->db->JOIN('cex_agenda_agenda_dia agd ','agdc.id_agenda_dia=agd.id_agenda_dia');
    ...
    function pacientes_espera_especialidad($id_especialidad,$id_departamento,
                                           $id_municipio,$tipo_atencion,$prioridad,
                                           $prioritaria,$id_entidad_remitente,$id_tiempo,
                                           $id_entidad,$cadena)
    {
        $this->db->SELECT("cr.id, ct.primer_nombre, 
                           ...
                           ct.numero_documento, cr.pin, cr.fecha_solicitud,
                           cr.tipo_cita, cr.pin");
        ...
        if($id_tiempo!="todas")
         ...
         if($cadena!="")
             $this->db->LIKE('medico_remite',$cadena);
        $this->db->ORDERBY('cr.fecha_solicitud ASC');
...
//////////////////////////////////////////////////////////////////////////////
    function datos_basicos_cita($id_cita)
    {
        $query="select cr.`fecha_solicitud`,
                ...
                cr.prioritaria,cr.medico_remite,
                ct.`celular`, ct.`email`, ct.`fecha_nacimiento`,
                ct.`telefono`,cr.`tipo_atencion`,cr.`tipo_cita`, cr.duracion_cita,
                cp.`genero`, ct.`telefono`, ct.`numero_documento`, cr.tipo_cita,cr.observaciones, cr.pin,
                cerp.`razon_social` eps, cr.id_paciente
      ...
      inner join `core_eapb` caep on cr.`id_entidad`=caep.`id_entidad`
      inner join core_tercero cerp on cerp.`id_tercero`=caep.`id_tercero`
      inner join core_tercero ct on ct.`id_tercero` = cr.`id_tercero`
      inner join `core_municipio` cm on cm.`id_municipio`=cr.`id_municipio`
      inner join `core_departamento` cd on cm.`id_departamento`=cd.`id_departamento`
...
///////////////////////////////////////////////////////////////////////////////
    function pacientes_programados_dia_medico($fecha, $id_medico)
    {
        $query="select cr.`id`, ct.`primer_nombre`, ct.`segundo_nombre`, ct.`primer_apellido`,
                ...
                cag.`orden_intervalo` intervalo_cita, cr.estado, ct.numero_documento, ct.celular,
                aad.fecha
                from `cex_citas_asignadas_agenda` cag
                inner join `cex_cita_remisiones` cr on cr.`id`=cag.`id_remision`
                inner join `cex_agenda_dia_detalle` agdd on agdd.`id_agenda_dia_detalle`=cag.`id_agenda_dia_detalle`
                inner join `core_tercero` ct on cr.`id_tercero`=ct.`id_tercero`
                inner join `cex_agenda_agenda_dia_consultorio` aggdc on aggdc.`id_agenda_consultorio`= agdd.`id_agenda_consultorio`
                inner join `cex_agenda_agenda_dia` aad on aad.`id_agenda_dia`= aggdc.`id_agenda_dia`
                where agdd.`id_especialista`=$id_medico and
                      aad.`fecha`='$fecha'
                order by agdd.orden_intervalo,cag.orden_intervalo, cr.id ";
        return $this->db->query($query)->result_array();
    }
...
///////////////////////////////////////////////////////////////////////////////
     function maximo_intervalo($id_detalle_agenda)
    {
        $query="select IFNULL( max(orden_intervalo),0) orden_intervalo
                FROM `cex_citas_asignadas_agenda`
...
function asignar_cita($id_detalle_agenda, $id_cita, $orden_intervalo,$duracion_cita,$id_usuario)
{
    ...
    $this ->db->insert('cex_citas_asignadas_agenda',$insert);
    $insert=array('id_agenda_dia_detalle'=>$id_detalle_agenda,
                  'id_cita'=>$id_cita,
                  'orden_intervalo'=>$orden_intervalo+1,
                  'duracion'=>$duracion_cita,
                ...
                from `cex_citas_asignadas_agenda` caa
                    where caa.`id_agenda_dia_detalle` = 
                                    (select caa2.`id_agenda_dia_detalle` 
                                     from cex_citas_asignadas_agenda caa2
