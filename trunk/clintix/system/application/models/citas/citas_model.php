...
///////////////////////////////////////////////////////////////////////////////
function buscar_citas_listas_usuario($d)
{
    $this->base_consulta_cita();
	
    $fecha_inicio=$d['fecha_agenda_inicial'];
    $fecha_fin=$d['fecha_agenda'];
  // $this->db->WHERE('cr.id_usuario',$d['id_usuario']);
	if($d['id_estado']=="asignada")
        $this->db->WHERE("aad.fecha BETWEEN '$fecha_inicio' AND '$fecha_fin'");
    else
        $this->db->WHERE("cr.fecha_solicitud BETWEEN '$fecha_inicio' AND '$fecha_fin'");
...
function observacion_agenda($id_cita)
{
	 $this->db->SELECT('cr.observacion_agendamiento,
                       ');
    $this->db->FROM('cex_cita_remisiones cr');
	
    $this->db->WHERE('cr.id',$id_cita);
	$result = $this->db->get();
    $num = $result -> num_rows();
    if($num == 0){
    return $num;}
    $res = $result -> row('observacion_agendamiento');
    return  $res;
...
{
    $this->db->WHERE('id',$id_cita);
    $update=array('observacion_agendamiento'=> $observacion);
    $this->db->UPDATE('cex_cita_remisiones',$update);
}
///////////////////////////////////////////////////////////////////////////////
function verificaExisteRemision($especialidad,$id_paciente)
{
    $this->db->SELECT('cr.id,
                       ');
    $this->db->FROM('cex_cita_remisiones cr');
	
    $this->db->WHERE('cr.id_especialidad',$especialidad);
	$this->db->WHERE('cr.id_paciente',$id_paciente);
	$this->db->WHERE('cr.estado <>','atendida');
	$this->db->WHERE('cr.estado <>','cancelada');
	$this->db->WHERE('cr.estado <>','rechazada');
	 $result = $this->db->get();
    $num = $result -> num_rows();
    if($num == 0){
    return $num;}
...
// buscar cita a modificar entidad

function buscar_cita_modificar($d)
{
    $this->base_consulta_cita();
...
///////////////////////////////////////////////////////////////////////////////
function cambiar_entidad_cita($d)
{
    $this->db->WHERE('pin',$d['pin']);
    $update=array('id_entidad'=> $d['id_entidad']);
    $this->db->UPDATE('cex_cita_remisiones',$update);
}
///////////////////////FIN/////////////////////////////////////////////////////
