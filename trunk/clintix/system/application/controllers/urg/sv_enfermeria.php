...
*/
	...
	function main($id_atencion)
	{
		...
		$d['notas'] = $this -> enfermeria_model -> obtenerSvNotas($id_atencion,$d['atencion']['id_servicio']);	
	 	$d['monitor'] = $this -> enfermeria_model -> obtenerMonitoria($id_atencion);
		// generamos el enlace para el boton volver
	...
	function crearNota_()
	{
		...
		//verificamos la hora para poder identificar el turno de la enfermera.
		 if ($hora > '00:00' && $hora < 07)
	     {
				 ...
				 $dato['existe'] = $this -> enfermeria_model -> VerificarExistencia($d['id_atencion'],$fecha_turno,$d['hora']);
		 }
		 	else{
      		 	$dato['existe'] = $this -> enfermeria_model -> VerificarExistencia($d['id_atencion'],$fecha_turno,$d['hora']);
			   }
		...
		if($dato['existe']==0)
		{
			$d['id_nota'] = $this -> enfermeria_model -> crearSvNotaDb($d,$dato['existe']);	
		}
			else{
			/*verificamos si ya se han almacenado los registros de los gases arteriales para la fecha y hora indicada
		 	si ya han sido almacenados saldra una alerta y no grabara este dato */
				if($dato['existe'][0]['signos_ingresados']=='SI')
					{
				$d['id_nota'] = $this -> enfermeria_model -> crearSvNotaDbMinute($d);
					}
					 else{
						 $existe=1;
						   $d['id_nota'] = $this -> enfermeria_model -> crearSvNotaDb($d,$existe);
						 }
...
function consultaMonitoria($id_atencion,$fecha_turno)
	{
		...
		//-----------------------------------------------------------
		$this->load->view('core/efecto_inicio');
		$this -> load -> view('urg/urg_svMonitoria', $d);
...
*/	
	function crearNotaGases($id_atencion)
	{
		...
		$this -> load -> view('urg/urg_svgases',$d);
...
///////////////////////////////////////////////////////////////////
	...
	/* 
* @Descripcion:  Crear un registro de glucometria.
* @author Diego Ivan Carvajal Gil <dcarvajal@opuslibertati.org>
* @author http://www.opuslibertati.org
* @copyright	GNU GPL 3.0
* @since		20121004
* @version		20121004
*/	
	function crearNotaGlucometria($id_atencion)
	{
		//---------------------------------------------------------------
		$d = array();
		//---------------------------------------------------------------
		...
		$this -> load -> view('urg/urg_svGlucometria',$d);
...
*/
	function crearNotaGases_()
	{
		...
		if($dato['existe']==0)
		{
			$d['id_nota'] = $this -> enfermeria_model -> crearSvNotaGasesDb($d,$dato['existe']);	
		}
			else{
			/*verificamos si ya se han almacenado los registros de los gases arteriales para la fecha y hora indicada
		 	si ya han sido almacenados saldra una alerta y no grabara este dato */
				if($dato['existe'][0]['gases_ingresados']=='SI')
					...
					//----------------------------------------------------------
						 ...
						 $existe=1;
						   $d['id_nota'] = $this -> enfermeria_model -> crearSvNotaGasesDb($d,$existe);
						 }
...
* @version		20111115
*/
	function crearNotaGlucometria_()
	{
		//---------------------------------------------------------------
		$d = array();
		$d['id_medico'] = $this->input->post('id_medico');
		...
		if($dato['existe']==0)
		{
			$d['id_nota'] = $this -> enfermeria_model -> crearSvNotaGlucometriaDb($d,$dato['existe']);	
		}
			else{
			/*verificamos si ya se han almacenado los registros de los gases arteriales para la fecha y hora indicada
		 	si ya han sido almacenados saldra una alerta y no grabara este dato */
				if($dato['existe'][0]['glucometria_ingresada']=='SI')
					{
						//----------------------------------------------------
					$dt['mensaje']  = "La glucometria para las".$d['hora']." horas ya han sido grabados anteriromente !!";
					$dt['urlRegresar'] 	= site_url("urg/sv_enfermeria/main/".$d['id_atencion']);
					$this -> load -> view('core/presentacionMensaje', $dt);
					return;	
					//----------------------------------------------------------
						 ...
						 $existe=1;
						   $d['id_nota'] = $this -> enfermeria_model -> crearSvNotaGlucometriaDb($d,$existe);
						 }
		...
		//----------------------------------------------------
		$dt['mensaje']  = "Los datos de Glucometria se han almacenado correctamente!!";
		$dt['urlRegresar'] 	= site_url("urg/sv_enfermeria/main/".$d['id_atencion']);
		$this -> load -> view('core/presentacionMensaje', $dt);
		return;	
		//----------------------------------------------------------
