...
<tr>
  <th colspan="2">Resultado Analizador</th></tr>
  <td colspan="2">
  <table>
  <tr> <td colspan="2"> <img id='logo'
src="data:image/jpg;base64,<?=$imagen[0]?>" /> </td> </tr>
  <tr><td class="campo">Codigo:</td>
 
  <td class="campo">Resultado:</td>
   </tr>
...
$total=count($analizador['nombre']);
for($i=0;$i < $total;$i++){
	echo form_hidden('codigo[]', $analizador['codigo'][$i]);
	
	echo form_hidden('valor[]', $analizador['valor'][$i]);
	?>
    <tr>
   <td><?=$analizador['codigo'][$i]?></td>
  
   <td><?=$analizador['valor'][$i]?></td> 
    </tr>
	<?
