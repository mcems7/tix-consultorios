...
{
	var var_url = '<?=site_url()?>/core/administrar_funcionario/obtenerDepartamento';
...
{
	var var_url = '<?=site_url()?>/core/administrar_funcionario/obtenerMunicipio';
...
{
	var var_url = '<?=site_url()?>/core/administrar_funcionario/noAplicaMunicipio';
	var ajax1 = new Request(
	{
		url: var_url,
		method: 'post',
...
function validarFormulario()
{
	...
	var depe = $('id_dependencia').value;
	
	if(depe == 0){
		alert("Debe seleccionar una dependencia de la lista!!");
		return false;}
...
$attributes = array('id'       => 'formulario',
	...
	echo form_open('/core/administrar_funcionario/editarFuncionario_',$attributes);
	echo form_hidden('id_tercero',$funcionario['id_tercero']);
	echo form_hidden('id_funcionario',$funcionario['id_funcionario']);
?>
<h1 class="tituloppal">Administración del sistema</h1>
<h2 class="subtitulo">Editar un funcionario</h2>
<center>
...
?>
 ...
 <tr>
    ...
    <?
		...
		{
			if($funcionario['id_dependencia'] == $d['id_dependencia'])
			{	
				echo '<option value="'.$d['id_dependencia'].'" selected="selected">'.$d['nombre_dependencia'].'</option>';
			}else{
   ...
   <tr>
    ...
    <td><?=form_input(array('name' => 'cargo',
							'id'=> 'cargo',
  ...
  <tr>
    ...
    <?php
		...
		if($funcionario['estado'] == 'Activo'){
			$res1 = 'checked="checked"';
		}else if($funcionario['estado'] == 'Inactivo'){
			$res2 = 'checked="checked"';
		}
	?>
 ...
 <tr>
    ...
    <td><input name="fecha_inicio" type="text" id="fecha_inicio" value="<?=$funcionario['fecha_inicio']?>" size="10" maxlength="10" class="calendario" readonly><img src="<?=base_url(); ?>resources/img/calendario_boton.png" id="fecha" title="Seleccionar fecha inicio" style="cursor:pointer" onclick="displayCalendar(document.formulario.fecha_inicio,'yyyy-mm-dd',this)" /></td>
  </tr>
 
  <td class="campo">Fecha de fin:</td>
    <td><input name="fecha_fin" type="text" id="fecha_fin" value="<?=$funcionario['fecha_fin']?>" size="10" maxlength="10" class="calendario" readonly><img src="<?=base_url(); ?>resources/img/calendario_boton.png" id="fecha" title="Seleccionar fecha fin" style="cursor:pointer" onclick="displayCalendar(document.formulario.fecha_fin,'yyyy-mm-dd',this)" /></td>
  </tr>
  ...
  <tr>
    ...
    <td><?=form_textarea(array('name' => 'observaciones',
								...
								'rows' => '3',
								'cols'=> '30',
								'value' => $funcionario['observaciones']))?></td>
  </tr>
