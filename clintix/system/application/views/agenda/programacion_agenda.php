...
return respuesta;
 ...
 function actualizarListaMedico1()
 {
    var var_url = '<?=site_url()?>/agenda/programar_agenda/listar_especialistas_por_especialidad1/'+$('id_especialidad1').value;
    var ajax1 = new Request(
	{       
		url: var_url,
                async: false,
		onSuccess: function(html){ 
                                    $('lista_medicos_consultorio1').set('html', html)
                                     },
		evalScripts: true,
		onFailure: function(){alert('Error verificando agenda');
                }
...
function actualizarConsultorios1()
{
    var var_url="";
    if($('medico_disponibilidad1').value=="")
        var_url = '<?=site_url()?>/agenda/programar_agenda/consultorios_especialidad1/-1';
    else   
        var_url = '<?=site_url()?>/agenda/programar_agenda/consultorios_especialidad1/'+$('medico_disponibilidad1').value;
    var ajax1 = new Request(
	{       
		url: var_url,
                async: false,
		onSuccess: function(html){ 
                    $('consultorios_especialidades1').set('html',html);
                    //$('lista_especialidades').set('html', html)
                           },
		evalScripts: true,
		onFailure: function(){alert('Error verificando agenda');
...
function bloqueo()
{
  var especialidad = $('medico_disponibilidad1').value
	
     if(confirm('Se bloqueara la agenda. ¿Esta seguro que desea continuar?'))
	{      
        
		
    var_url = '<?=site_url()?>/agenda/main/agregarBloqueo/'+ $('fecha_agenda').value+'/'+$('intervalo_inicial_eliminacion1').value+'/'+$('intervalo_final_eliminacion1').value+'/'+$('consultorio1').value+'/'+especialidad;
    var ajax1 = new Request(
	{       
		url: var_url,
                async: false,
...
function desbloqueo()
{
  var especialidad = $('medico_disponibilidad1').value
     ...
     if(confirm('Se quitara el bloqueo de la agenda. ¿Esta seguro que desea continuar?'))
	{      
        
		
    var_url = '<?=site_url()?>/agenda/main/agregarDesbloqueo/'+ $('fecha_agenda').value+'/'+$('intervalo_inicial_eliminacion1').value+'/'+$('intervalo_final_eliminacion1').value+'/'+$('consultorio1').value+'/'+especialidad;
    var ajax1 = new Request(
	{       
		url: var_url,
                async: false,
...
<input type="radio" name="acciones" value="bloquear" onClick="mostrar_bloqueo()"> Bloquear Items Agenda
    ...
    <td> 
        <table>
            <tr>
                ...
                <td class="campo">Hora Final:</td><td id="hora_seleccionada"><?=form_dropdown('intervalo_final',$horarios_hasta,'','id="intervalo_final"');?></td>
                <td class="campo">Consultorio:</td><td id="consultorios_especialidades"></td>
            </tr>
        </table>
    </td>
</tr>
<td class="campo_izquierda"><?=form_submit('boton', 'Agregar')?> <?=form_submit('boton', 'Sustituir')?></td>
</table>
 <?=form_close();?>
    <script>
    actualizarListaMedico();
...
<tr><td colspan="2">
    <table>
            <tr>
                ...
                <td class="campo">Hora Final:</td><td id="hora_seleccionada"><?=form_dropdown('intervalo_final_eliminacion',$horarios_hasta,'','id="intervalo_final_eliminacion"');?></td>
...
<tr>
  <td><table><tr>
    <td class="campo_izquierda">Especialidad:</td><td id="lista_especialidadess1"><?=form_dropdown('id_especialidad1',$listadoEspecialidades1,'-1','id="id_especialidad1" onChange="actualizarListaMedico1()"')?></td>
  </td></tr> </table>
</tr>
<tr>
  <td><table><tr>
     <td class="campo_izquierda">Especialista:</td><td id="lista_medicos_consultorio1"></td></tr>
  </td></tr></table>
</tr>
<tr>
    <td> 
        <table>
            <tr>
             <td class="campo">Hora Inicio:</td><td id="hora_seleccionada"><?=form_dropdown('intervalo_inicial_eliminacion',$horarios,'','id="intervalo_inicial_eliminacion1"');?></td>
                <td class="campo">Hora Final:</td><td id="hora_seleccionada"><?=form_dropdown('intervalo_final_eliminacion',$horarios_hasta,'','id="intervalo_final_eliminacion1"');?></td>
               
                <td class="campo">Consultorio:</td><td id="consultorios_especialidades1"></td>
            </tr>
        </table>
    </td>
</tr>
<td class="campo_izquierda"><? $js = 'onClick="bloqueo()"';

echo form_submit('boton', 'Bloquear', $js);?>  <? $js = 'onClick="desbloqueo()"';

echo form_submit('boton', 'Desbloquear', $js);?></td>
</table>
 <?=form_close();?>
    <script>
    actualizarListaMedico1();
    actualizarConsultorios1();
    </script>
</div>
