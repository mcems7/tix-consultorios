...
function open_flash_chart_object_str( $width, $height, $url, $use_swfobject=true, $base='sdasdasd' )
{
    //
...
function open_flash_chart_object( $width, $height, $url, $use_swfobject=true, $base='asdasd' )
{
    //
...
function _ofc( $width, $height, $url, $use_swfobject, $base )
{
    ...
    if( !isset( $open_flash_chart_seqno ) )
    {
        ...
        $out[] = '<script type="text/javascript" src="'. $base .'resources/assets/js/swfobject.js"></script>';
    }
    else
    {
	...
	$out[] = 'var so = new SWFObject("'. $base .'resources/assets/swf/open-flash-chart.swf", "'. $obj_id .'", "'. $width . '", "' . $height . '", "9", "#FFFFFF");';
