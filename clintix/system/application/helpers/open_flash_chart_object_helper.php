<?php
/**
 * Adaptación de la librería provista por Open Flash Chart en http://teethgrinder.co.uk/open-flash-chart/
 * para Codeigniter realizada por Alfonso Rubalcava http://www.trozosdecodigo.com/2008/04/19/open-flash-chart-y-codeigniter/
 *Todas las funciones son del autor original, solamente cambie algunas rutas para adaptarlas a la instalación descrita en el enlace citado...
 */
...
function _ofc( $width, $height, $url, $use_swfobject, $base )
{
	...
	$out[] = 'var so = new SWFObject("'. base_url() .'assets/js//open-flash-chart.swf", "'. $obj_id .'", "'. $width . '", "' . $height . '", "9", "#FFFFFF");';
    ...
    $out[] = '<param name="movie" value="'. base_url() .'system/ofcharts/open-flash-chart.swf?width='. $width .'&height='. $height . '&data='. $url .'" />';
    $out[] = '<param name="quality" value="high" />';
    $out[] = '<param name="bgcolor" value="#FFFFFF" />';
    $out[] = '<embed src="'. base_url() .'system/ofcharts/open-flash-chart.swf?data=' . $url .'" quality="high" bgcolor="#FFFFFF" width="'. $width .'" height="'. $height .'" name="'. $obj_id .'" align="middle" allowScriptAccess="sameDomain" ';
