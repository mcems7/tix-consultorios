...
function diferenciafecha($fecha)
{
 ...
 return $this->db->query('SELECT TIMESTAMPDIFF(MINUTE,"'.$fecha.'" , "'.$fecha_actual.'") AS minutos')->result_array();
...
{
		$this->db->select('urg_evoluciones.subjetivo enfermedad_actual,
  urg_evoluciones.analisis analisis');
  		$this->db->from('urg_evoluciones');
		$this->db->where('id_atencion',$id_atencion);
		$this->db->order_by('urg_evoluciones.id_atencion','DESC');
	$this->db->limit(1);
		$result = $this->db->get();
...
function obtenercarpetahc($numero_documento)
{
	$this->db->select('hc_carpeta.tiene_carpeta,hc_carpeta.numero_carpeta');
  		$this->db->from('hc_carpeta');
		$this->db->where('cedula',$numero_documento);
	    $this->db->limit(1);
		$result = $this->db->get();
		return $result->row_array();
...
///////////////diego carvajal impresion historia clinica consulta
function obtenercontenedorpadre($id_cita)
	{
			$this->db->select('cex_atencion_item_hc.idpadre As contenedor');
			$this->db->from('cex_atencion_hce');
			$this->db->join('cex_atencion_item_hc','cex_atencion_hce.id_item_hc = cex_atencion_item_hc.id');

			$this->db->where('cex_atencion_hce.id_cita',$id_cita);
			$this->db->group_by('cex_atencion_item_hc.idpadre');
	...
	{
			$this->db->select('	
		    cex_atencion_item_hc.nombre,
			cex_atencion_item_hc.id');
			$this->db->from('cex_atencion_item_hc');

			$this->db->where('cex_atencion_item_hc.id',$contenedor);
			
			$this->db->group_by('cex_atencion_item_hc.id');
...
//////////////
	function OrdenInterpretar($id_cita)
	{
		$this->db->select('	
		cex_atencion_hce.id_item_hc,
		cex_atencion_hce.valor,
		cex_atencion_hce.id_cita,
		 cex_atencion_item_hc.nombre As clinico,
